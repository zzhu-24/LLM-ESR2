...
class Seq2SeqDatasetAllUser(Seq2SeqDataset):
    ...
    def __init__(self, args, data, item_num, max_len, neg_num=1):
        ...
        if self.args.enable_id:
            ...
            self.var_name.append("sim_collab_seq")
            self.var_name.append("sim_collab_positions")
    ...
    def __getitem__(self, index):
        ...
        if self.args.enable_id:
            ...
            for sim_user in sim_collab_users:
                meta_seq, meta_positions = self._get_user_seq(sim_user)
                sim_collab_seq.append(meta_seq)
                sim_collab_positions.append(meta_positions)
            
            sim_collab_seq = np.array(sim_collab_seq)
            sim_collab_positions = np.array(sim_collab_positions)

            return seq, pos, neg, positions, index, sim_seq, sim_positions, sim_collab_seq, sim_collab_positions
